...
void aide() {
  ...
  fprintf(stderr,
          "i         :   afficher la liste des emplacements memoire "
          "inoccupes\n");
  fprintf(stderr,
          "o         :   afficher la liste des emplacements memoire "
          "occupes\n");
  fprintf(stderr,
          "M         :   afficher la liste de tous les emplacements "
...
void afficher_zone_libre(void *adresse, size_t taille, int free) {
  if(free) afficher_zone(adresse, taille, 1);
}

void afficher_zone_occupee(void *adresse, size_t taille, int free) {
  if(!free) afficher_zone(adresse, taille, 0);
}
  ...
  while(1) {
    ...
    switch(commande) {
      ...
      case 'm':
        ...
        for(i = 0; i < mem_space_get_size(); i++) printf("%d ", adresse[i]);
        printf("]\n");
        break;
      case 'h':
        aide();
